<template>
  <div class="planet-page">
    ...
    <div class="top-toolbar">
      <button @click="handleBack" class="btn-back">← 返回宇宙</button>

      <div class="planet-title">
        <h2 v-if="currentPlanet">
          {{ currentPlanet.name || currentPlanet.id }}
        </h2>
      </div>

      <div class="toolbar-actions">
        <button
          @click="showProductionPanel = !showProductionPanel"
          class="btn-tool"
        >
          📊 生产
        </button>
      ...
      <div class="sidebar">
        ...
        <div v-if="showProductionPanel" class="production-panel">
          <div class="panel-header">
            <h3>生产统计</h3>
            <button @click="showProductionPanel = false" class="btn-close">
              ✕
            </button>
          </div>
          ...
          <div class="production-stats">
            <div class="stat-section">
              ...
              <div class="stat-item">
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.minerals) }}/秒</span
                >
              </div>
              <div class="stat-item">
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.energy) }}/秒</span
                >
              </div>
              <div class="stat-item">
                <span class="icon">🌾</span>
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.food) }}/秒</span
                >
              </div>
            </div>
            ...
            <div class="stat-section">
              ...
              <div class="stat-item">
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.alloys) }}/秒</span
                >
              </div>
              <div class="stat-item">
                <span class="icon">🔋</span>
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.powerCells) }}/秒</span
                >
              </div>
              <div class="stat-item">
                <span class="icon">📦</span>
                ...
                <span class="value"
                  >+{{ formatNumber(planetProduction.consumerGoods) }}/秒</span
                >
              </div>
            </div>
...
const totalBuildings = computed(
  () => currentPlanet.value?.buildings.length || 0
);
const activeBuildings = computed(
  () =>
    currentPlanet.value?.buildings.filter((b) => b.status === "active")
      .length || 0
);
const buildingInProgress = computed(
  () =>
    currentPlanet.value?.buildings.filter(
      (b) => b.status === "building" || b.status === "upgrading"
    ).length || 0
);
...
async function handleUpgrade(buildingId: string) {
  ...
  const building = currentPlanet.value.buildings.find(
    (b) => b.id === buildingId
  );
  if (!building) return;
